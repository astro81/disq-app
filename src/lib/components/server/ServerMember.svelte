...
<div>
    <button 
        onclick={() => goto(`/servers/${page.params.serverId}/conversations/${member.memberId}`)}
        class={cn(
    ...
    >
        ...
        <p class={cn(
            ...
            // page.params.memberId === member.memberId  && "text-primary dark:text-zinc-200 dark:group-hover:text-white"
        )}>
            {member.username}
        </p>
