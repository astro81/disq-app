...
export const PRELOADED_LANGS = [
    'javascript', 'typescript', 'python', 'svelte', 'rust', 
    'tsx', 'html', 'css', 'cpp', 'csharp', 'c', 
    'docker', 'json', 'jsx', 'lua', 'nix', 'razor'
] as BundledLanguage[]
...
export const PRELOADED_THEMES = [
    ...
    'catppuccin-mocha', 'dracula', 'dracula-soft', 'gruvbox-dark-soft',
    'kanagawa-dragon', 'tokyo-night'
] as BundledTheme[]
...
export async function highlightCode(code: string, lang: string, theme: string) {
    ...
    const validatedLang = PRELOADED_LANGS.includes(lang as BundledLanguage) 
        ? lang 
        : 'text'
        
    // Fallback for themes not in your list
    const validatedTheme = PRELOADED_THEMES.includes(theme as BundledTheme)
        ? theme
        : 'github-dark'
