...
const audioAnalysers = new SvelteMap<string, { analyser: AnalyserNode; interval: ReturnType<typeof setInterval> }>()
...
export async function joinVoiceChannel(
  ...
  try {
    ...
    //* log local strean 
    console.log('[Voice] tracks:', _localStream.getTracks().map(t => `${t.kind}:${t.label}:${t.enabled}`))
  ...
  // 2. Add self as local participant
  _participants = [{
  ...
  const peersUnsub = onSnapshot(
    collection(db, 'voice-calls', channelId, 'peers'),
    (snap) => {
  ...
  // 6. Watch offers addressed to us
  const offersUnsub = onSnapshot(
    collection(db, 'voice-calls', channelId, 'offers'),
    (snap) => {
...
// ─── Offer / Answer (audio only, same signalling pattern) ─────────
