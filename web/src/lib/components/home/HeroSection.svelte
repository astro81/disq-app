...
<section
	bind:this={container}
	class="relative min-h-screen overflow-hidden pt-24 pb-20 sm:pt-32 sm:pb-32 flex items-center"
>
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
		<div class="mx-auto max-w-4xl text-center">
			...
			<div
				...
				class="text-5xl font-bold tracking-tight text-foreground sm:text-7xl lg:text-8xl leading-[0.9]"
			...
			<div
				...
				class="block text-5xl font-bold tracking-tight sm:text-7xl lg:text-8xl mt-2 leading-[0.9] cursor-pointer"
			...
			<p
				bind:this={desc}
				class="mx-auto mt-8 max-w-2xl text-lg text-muted-foreground sm:text-xl lg:text-2xl leading-relaxed"
			>
				Real-time messaging meets beautiful code snippets. Built for teams who collaborate, debug,
				and ship together.
			</p>

			<div bind:this={cta} class="mt-12 flex flex-col items-center justify-center gap-4 sm:flex-row">
				<button
					onclick={() => goto(resolve('/servers/@me'))}
					class="inline-flex items-center justify-center gap-2 w-full sm:w-auto h-14 px-8 text-base rounded-md bg-primary text-primary-foreground font-medium group transition-all duration-300 hover:scale-105 hover:shadow-lg hover:bg-primary/90"
	...
	<div
		...
		class="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 cursor-pointer transition-opacity hover:opacity-70"
		onclick={scrollDown}
		role="button"
		tabindex="0"
		onkeydown={(e) => e.key === 'Enter' && scrollDown()}
	>
		<span class="text-xs text-muted-foreground uppercase tracking-[0.2em] font-medium">Explore</span>
