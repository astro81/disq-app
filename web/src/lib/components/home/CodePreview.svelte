		...
		ctx = gsap.context(() => {
			...
			gsap.fromTo(
				...
				{ opacity: 0, x: -50 },
				{
					...
					scrollTrigger: { trigger: sectionEl, start: 'top 50%', toggleActions: 'play none none reverse' }
				}
			);
			...
			if (messages) {
				gsap.fromTo(
					...
					{
						opacity: 1, y: 0, x: 0, scale: 1, duration: 0.6, stagger: 0.15, ease: 'back.out(1.5)',
						scrollTrigger: { trigger: sectionEl, start: 'top 40%', toggleActions: 'play none none reverse' }
					}
				);
			}
			...
			gsap.fromTo(
				...
				{
					opacity: 1, scale: 1, boxShadow: '0 0 60px 0 rgba(94, 234, 212, 0.15)', duration: 1, ease: 'power3.out',
					scrollTrigger: { trigger: codeBlock, start: 'top 60%', toggleActions: 'play none none reverse' }
				}
			);
...
<section bind:this={sectionEl} class="relative py-32">
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		<div bind:this={title} class="text-center mb-16">
							...
							<div class="space-y-1">
								{#each channels as channel (channel)}
									<div
										...
										class="rounded-lg px-3 py-2.5 text-sm transition-all duration-300 cursor-pointer {channel === activeChannel
						...
						<div>
							...
							<div class="space-y-1">
								{#each voiceChannels as channel (channel)}
									<div
										class="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-2.5 text-sm text-muted-foreground transition-all duration-300 hover:bg-secondary hover:text-foreground hover:translate-x-1"
									>
					...
					<div class="flex flex-1 flex-col">
						<div class="flex items-center justify-between border-b border-border px-5 py-4">
							<div class="flex items-center gap-3">
								...
								<span class="text-sm text-muted-foreground hidden sm:inline">Team discussions</span>
							</div>
						</div>

						<!-- Messages -->
						<div bind:this={messages} class="flex-1 space-y-5 p-5 overflow-y-auto">
							<!-- Message 1 -->
									...
									>
										<div class="flex items-center justify-between border-b border-border px-4 py-2.5">
											<div class="flex items-center gap-3">
												...
												<span
													class="rounded-md bg-accent/20 px-2 py-0.5 text-[10px] font-semibold text-accent uppercase tracking-wide"
													>TypeScript</span
												>
											</div>
											<button
												onclick={handleCopy}
												class="flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs text-muted-foreground transition-all duration-300 hover:bg-secondary hover:text-foreground hover:scale-105"
											>
												{#if copied}
										...
										<pre class="overflow-x-auto p-5 text-sm leading-relaxed"><code class="font-mono"><span class="text-blue-400">async function</span> <span class="text-accent">deployToProduction</span><span class="text-foreground">() {'{'}</span>
  <span class="text-blue-400">const</span> <span class="text-foreground">config</span> <span class="text-foreground">=</span> <span class="text-blue-400">await</span> <span class="text-accent">loadConfig</span><span class="text-foreground">();</span>

  <span class="text-blue-400">if</span> <span class="text-foreground">(!config.apiKey) {'{'}</span>
    <span class="text-blue-400">throw new</span> <span class="text-orange-400">Error</span><span class="text-foreground">(</span><span class="text-green-400">'Missing API key'</span><span class="text-foreground">);</span>
  <span class="text-foreground">{`}`}</span>

  <span class="text-blue-400">const</span> <span class="text-foreground">result</span> <span class="text-foreground">=</span> <span class="text-blue-400">await</span> <span class="text-accent">deploy</span><span class="text-foreground">({'{'}</span>
    <span class="text-foreground">environment:</span> <span class="text-green-400">'production'</span>
  <span class="text-foreground">{'}'});</span>

  <span class="text-blue-400">return</span> <span class="text-foreground">result.status;</span>
<span class="text-foreground">{'}'}</span></code></pre>
									</div>
								</div>
							</div>
						...
						<div class="border-t border-border p-4">
							<div
								class="flex items-center gap-3 rounded-xl border border-border bg-secondary/30 px-4 py-3.5 transition-all duration-300 hover:border-accent/30 focus-within:border-accent/50 focus-within:ring-2 focus-within:ring-accent/10"
							>
								<MessageSquare class="h-5 w-5 text-muted-foreground" />
								<span class="flex-1 text-sm text-muted-foreground">Message {activeChannel}</span>
								<Send class="h-5 w-5 text-muted-foreground transition-colors hover:text-accent cursor-pointer" />
							</div>
						</div>
					</div>
				</div>
