...
<script lang="ts">
    ...
    let systemTheme = $derived.by(() => (mode.current === "light") ? 'catppucing-latte' : 'tokyo-night');
    ...
    $effect(() => {
        ...
        highlightCode(currentCode, currentLang, currentTheme)
            .then(html => { highlightedHtml = html })
            .catch(() => {
...
>
    ...
    >
        <div class="flex justify-start items-center gap-2">
            <span class="text-[11px] font-medium {isMine ? 'text-indigo-300' : 'text-zinc-500 dark:text-zinc-400'}">
                {langLabel}
            </span>

            <span class="text-[10px] opacity-60 font-medium {isMine ? 'text-indigo-300' : 'text-zinc-400 dark:text-zinc-500'}">
                {themeLabel}
            </span>
        </div>
    ...
    <div class="overflow-x-auto text-[13px] leading-6">
        ...
        {:else}
            ...
            <pre
                class="m-0 whitespace-pre overflow-x-auto px-4 py-3 font-mono
                       {isMine ? 'text-indigo-200' : 'text-zinc-800 dark:text-zinc-200'}"
            >{code}</pre>
        {/if}
    </div>
</div>
