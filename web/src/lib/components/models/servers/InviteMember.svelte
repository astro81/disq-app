<script lang="ts">
    ...
    const regenerateInviteCode = async (serverId: string): Promise<void> => {
        if (!serverId) {
        ...
        isLoading = true;
        try {
            const response = await fetch(`/api/servers/regenerateInvite/${serverId}`, {
                method: 'PATCH',
            });

            if (!response.ok) {
                const err = await response.json();
                alert(err.error ?? "Failed to regenerate invite link.");
                return;
            }
            ...
            console.error("Error regenerating invite code:", e);
    ...
    <DialogContent class="sm:max-w-131.25">
        ...
        <div class="mt-2">
            ...
            <Button
                ...
                onclick={() => regenerateInviteCode(currentServer.serverId)}
            >
                Generate a new link
