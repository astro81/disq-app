<!-- ChatInput.svelte -->
<script lang="ts">
  	import { sendMessage, sendFile, sendMessageWithFile, socketState, type FileAttachment } from '$lib/stores/socket.svelte'
...
<div class="flex flex-col border-t border-neutral-200 dark:border-neutral-800">
	...
	<!-- Error toast -->
    {#if fileError}
        <div class="mx-4 mt-2 flex items-center gap-2 rounded-md border border-destructive/20
                    bg-destructive/10 px-3 py-2 text-xs font-medium text-destructive
    ...
    {#if pendingFile}
        <div class="mx-4 mt-2 flex items-center gap-2.5 rounded-xl border border-zinc-200
                    bg-zinc-50 px-3 py-2 dark:border-zinc-700 dark:bg-zinc-800/60
                    animate-in fade-in slide-in-from-bottom-1">

            {#if isPendingImage}
                <!-- Thumbnail for images -->
                <div class="size-10 shrink-0 overflow-hidden rounded-lg border border-zinc-200 dark:border-zinc-600">
                    <img
                        src={pendingFile.url}
            ...
            <button
                ...
                class="ml-1 shrink-0 rounded-full p-1 text-zinc-400
                       ...
                       dark:hover:bg-zinc-700 dark:hover:text-zinc-200 transition-colors"
            >
                <X class="size-3.5" />
            </button>
        </div>
	...
	<!-- Input row -->
  	<div class="flex items-center gap-2 px-4 py-3">
        ...
        <button
            ...
            class="flex size-9 shrink-0 items-center justify-center rounded-lg transition-colors
                   text-zinc-500 hover:bg-zinc-100 hover:text-indigo-500
