...
<script lang="ts">
	...
	function parseCodeSnippet(text: string): { code: string; language: string, theme: string } | null {
		...
		return {
    	    ...
    	    theme:    match[2] ?? (window.matchMedia('(prefers-color-scheme: dark)').matches
    	                  ? 'tokyo-night'
    	                  : 'catppuccin-latte'),
    	    code:     match[3],
    	}
...
<div
  	...
  	class="flex flex-1 flex-col overflow-y-auto px-4 py-2 scroll-smooth"
>
  	...
  	{#each groupedMessages as dateGroup (dateGroup.dateLabel)}
  	  	<div class="my-4 flex items-center gap-3">
  	  	  	...
  	  	  	<span class="rounded-full bg-zinc-100 px-3 py-0.5 text-[11px] font-medium
  	  	...
  	  	{#each dateGroup.bubbleGroups as bubbleGroup (bubbleGroup.memberId + bubbleGroup.msgs[0]?.messageId)}
				...
				<div class="mt-auto shrink-0">
				    {#if !isMine}
				        ...
				        >
				            <button class="size-8 overflow-hidden rounded-full bg-zinc-700 cursor-pointer
				                           ring-2 ring-transparent hover:ring-indigo-500 transition-all">
				                {#if bubbleGroup.userProfileImage}
				                    <img src={bubbleGroup.userProfileImage} alt={bubbleGroup.displayName} class="size-full object-cover" />
				                {:else}
				                    <div class="flex size-full items-center justify-center text-xs font-bold text-white">
                    ...
                    {@const hasText = !!msg.message && !snippet}
                        ...
                        {#if hasFile}
                            {#if isImageFile}
                                <div class="group relative mb-0.5 overflow-hidden rounded-xl shadow-sm
                                            {isMine ? 'rounded-br-sm' : 'rounded-bl-sm'}
                                            max-w-xs border border-zinc-200 dark:border-zinc-700">
                                    <img src={msg.messageFileUrl} alt={msg.messageFileName ?? 'Image'}
                                         class="block max-h-64 w-full object-cover" loading="lazy" />
                                    <a href={resolve(msg.messageFileUrl)} download={msg.messageFileName}
                                       target="_blank" rel="noopener noreferrer" aria-label="Download image"
                                       class="absolute inset-0 flex items-center justify-center
                                        ...
                                        <div class="rounded-full bg-white/90 p-2 shadow">
											...
											{#if msg.messageFileSize != null}
    										    <span class="opacity-0 group-hover:opacity-100 transition-opacity text-red-600">
                            ...
                            {:else}
                                <a href={resolve(msg.messageFileUrl)} download={msg.messageFileName}
                                   ...
                                   class="group mb-0.5 flex items-center gap-2.5 rounded-xl border px-3 py-2
                                          shadow-sm transition-colors no-underline
                                          {isMine
                                              ? 'border-indigo-500/40 bg-indigo-700 text-white hover:bg-indigo-600 rounded-br-sm'
                                              : 'border-zinc-200 bg-white text-zinc-800 hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100 dark:hover:bg-zinc-600 rounded-bl-sm'}">
                                    <div class="flex size-8 shrink-0 items-center justify-center rounded-md
                                                {isMine ? 'bg-indigo-500/40' : 'bg-zinc-100 dark:bg-zinc-600'}">
                                        <FileIcon class="size-4 {isMine ? 'text-white' : 'text-zinc-500 dark:text-zinc-300'}" />
                                    </div>
                                    <div class="min-w-0">
                                        <p class="truncate text-xs font-medium max-w-40">{msg.messageFileName ?? 'File'}</p>
                                        <p class="flex items-center gap-1 text-[10px] opacity-60">
                                            <Download class="size-2.5" />
                                            Download
											{#if msg.messageFileSize != null}
    										    <span class="opacity-0 group-hover:opacity-100 transition-opacity text-amber-500">
            									    · {formatBytes(msg.messageFileSize)}
            									</span>
    										{/if}
                                        </p>
                        ...
                        <!-- ── Plain text bubble ── -->
                        {#if hasText}
  	  	      	  	        <div class="px-4 py-2 text-sm leading-relaxed
  	  	      	  	  	               {isMine
